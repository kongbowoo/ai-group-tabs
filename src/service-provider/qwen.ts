...
export const fetchQwen = async (
  apiKey: string,
  tabInfo: TabInfo,
  types: string[]
) => {
  // Alibaba Cloud DashScope (Qwen) API is compatible with OpenAI API format
  // See https://help.aliyun.com/zh/dashscope/developer-reference/compatibility-of-openai-compatible-api/
  const apiURL =
    (await getStorage("apiURL")) ||
    "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";

  const model = (await getStorage("model")) || "qwen-turbo";
